...
- **GitLab mode** (2026-08): a file can instead come from a GitLab repo — `state.remote` (`{base, projectId, projectPath, branch, path, lastCommitId}`) replaces `state.fileHandle`. All calls go through `glApi(path, opts, base)` straight from the browser (needs API CORS on self-hosted instances). **Tokens are per instance**: `gitlab-tokens` maps base → PAT (legacy single `gitlab-token` still migrates), `gitlab-base` only remembers the one used last — every remote call must pass its file's own `r.base`, or a recent from instance A gets fetched against instance B with B's token. Save = commit to the opened branch with `last_commit_id` as optimistic lock (400 → conflict banner), then re-reads the head and compares content, since a foreign commit can land between PUT and GET. Watching polls the commits API every 15s (vs 3s for disk). Every open/save/watch/recents path must handle both `state.fileHandle` and `state.remote`.
